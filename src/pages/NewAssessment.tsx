...
const NewAssessment = () => {
  ...
  const downloadScript = () => {
    const uploadEndpoint = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/upload-data`;
...
function Get-TrustRelationships {
    ...
    try {
        ...
        Write-Host "[!] Error collecting trusts: $_" -ForegroundColor Red
        return @()
    }
}
...
function Get-AllADGroups {
    ...
    try {
        $groups = Get-ADGroup -Filter * -Properties Members, Description | ForEach-Object {
            ...
            $privilegeLevel = $null
            if ($_.Name -in @("Domain Admins", "Enterprise Admins", "Schema Admins", "Administrators")) {
                $privilegeLevel = "TIER0"
            } elseif ($_.Name -in @("Account Operators", "Backup Operators", "Server Operators")) {
        ...
        if ($gpModuleAvailable) {
            $gpos = Get-GPO -All | ForEach-Object {
                ...
                try {
                    ...
                    $linksTo = ([xml]$gpoReport).GPO.LinksTo
...
function Get-LAPSStatus {
    ...
    try {
        ...
        return @{ SchemaExtended = $false; GPOConfigured = $false; ComputersWithLAPS = 0 }
    }
}
...
function Get-DNSConfiguration {
    Write-Host "\`n[*] Analyzing DNS Configuration..." -ForegroundColor Green
    try {
        $dnsInfo = @{
            Zones = @()
            Forwarders = @()
        ...
        if ($dnsModuleAvailable) {
            foreach ($dc in $dcs) {
                try {
                    ...
                    Write-Host "[!] Could not query DNS on $($dc.Name)" -ForegroundColor Yellow
                }
            }
        } else {
            $dnsInfo.Note = "Install DNSServer module for detailed DNS analysis"
        }

        Write-Host "[+] DNS configuration collected: $($dnsInfo.Zones.Count) zones" -ForegroundColor Green
        return $dnsInfo
    } catch {
...
try {
    ...
    try {
        $jsonPayload | Out-File -FilePath $localFilePath -Encoding UTF8
    ...
    if ($OfflineMode) {
        ...
        Write-Host "[*] Copy this file to a machine with internet and upload via web interface" -ForegroundColor Yellow
    } else {
        ...
        $response = Invoke-RestMethod -Uri $ApiEndpoint -Method POST -Body $jsonPayload -Headers $headers -TimeoutSec 300
