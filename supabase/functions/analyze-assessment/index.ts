...
// Configuration limits for data processing - AGGRESSIVE OPTIMIZATION
const MAX_USERS_PER_CATEGORY = 30; // Reducido de 100 a 30
const MAX_GPOS = 20; // Reducido de 50 a 20
const MAX_DATA_SIZE_CHARS = 80000; // Reducido de 150KB a 80KB
const MAX_FIELD_LENGTH = 200; // Truncar campos largos
...
async function performAnalysis(assessmentId: string) {
  try {
    ...
    if (assessment.file_path) {
      console.log('Loading data from storage:', assessment.file_path);
      try {
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      try {
        ...
        if (!categoryData || categoryData.length === 0) {
          console.log(`No data for category: ${category.id}, skipping`);
          continue;
        }
      } catch (error) {
...
// Strategic sampling: prioritize critical users - ULTRA AGGRESSIVE
function sampleUsers(users: any[]): any[] {
  ...
  users.forEach(user => {
    ...
    if (isPrivileged || isServiceAccount || hasOldPassword || isKerberoastable || !isASREPRoastable) {
      priorityUsers.push(simplifyUser(user)); // SIMPLIFIED
    } else if (normalUsers.length < 10) { // Solo 10 usuarios normales max
      normalUsers.push(simplifyUser(user)); // SIMPLIFIED
    }
  });
  
  // Take priority users (max 30) + very few normal users
  const maxPriority = Math.min(priorityUsers.length, MAX_USERS_PER_CATEGORY - 5);
  const maxNormal = Math.min(normalUsers.length, 5);
...
// Sample GPOs strategically - ULTRA AGGRESSIVE
function sampleGPOs(gpos: any[]): any[] {
  ...
  gpos.forEach(gpo => {
    ...
    if (isDefault) {
      priorityGPOs.push(simplifyGPO(gpo)); // SIMPLIFIED
    } else if ((hasMultipleLinks || hasWidePermissions) && priorityGPOs.length < 10) {
      priorityGPOs.push(simplifyGPO(gpo)); // SIMPLIFIED
    } else if (normalGPOs.length < 5) {
      normalGPOs.push(simplifyGPO(gpo)); // SIMPLIFIED
    }
  });
  
  return [...priorityGPOs.slice(0, 15), ...normalGPOs.slice(0, 5)]; // Max 20 GPOs total
}
